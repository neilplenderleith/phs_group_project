...
```{r}
wait <- waiting_times %>% 
  mutate("Admission to Same Facility" = (discharge_destination_admission_to_same/number_of_attendances_aggregate))

col1 <- sym("Admission to Same Facility")

wait %>% 
  filter(!is.na(!!col1)) %>% 
  group_by(date) %>% 
  summarise(mean_admission_to_same = mean(!!col1)) %>% 
  ggplot(aes(x = date,
